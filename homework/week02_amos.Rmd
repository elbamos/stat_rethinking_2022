...
samp1 |> 
  ...
  mutate(
    weight = rnorm(n(), alpha + beta * (height - mean(howell_adults$height)), sd = sigma)
  ) |> 
  group_by(individual, height) |>
  summarize(
    expectation = mean(weight), 
    l_interval = HPDI(weight, .89)[1], 
    r_interval = HPDI(weight, .89)[2]
  ) |> knitr::kable()
```
